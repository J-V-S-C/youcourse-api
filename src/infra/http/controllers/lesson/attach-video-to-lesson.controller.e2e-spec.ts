...
describe('Attach Video to Lesson (E2E)', () => {
  ...
  test('[POST] /lessons/:lessonId/video - Success', async () => {
    ...
    expect(response.body.video).toHaveProperty('externalId');
    expect(response.body.video).toHaveProperty('uploadUrl');
  });

  test('[POST] /lessons/:lessonId/video - Lesson not found', async () => {
    const user = await accountFactory.makePrismaAccount();
    const accessToken = jwt.sign({ sub: user.id.toString() });

    const response = await request(app.getHttpServer())
      .post('/lessons/non-existent-lesson/video')
      .set('Authorization', `Bearer ${accessToken}`)
      .send({
        filename: 'video.mp4',
        contentType: 'video/mp4',
  ...
  test('[POST] /lessons/:lessonId/video - Not the course creator', async () => {
