...
describe('Hide Course (E2E)', () => {
  ...
  test('[PATCH] /courses/:courseId/hide - Success', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch(`/courses/${course.id.toString()}/hide`)
      .set('Authorization', `Bearer ${accessToken}`)
      .send();

    expect(response.statusCode).toBe(204);

    const hiddenCourse = await prisma.course.findUnique({
      where: {
        id: course.id.toString(),
      },
    });

    expect(hiddenCourse?.visible).toBe(false);
  });
});
