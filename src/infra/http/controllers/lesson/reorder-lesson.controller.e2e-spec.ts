...
describe('Reorder Lesson (E2E)', () => {
  ...
  test('[PATCH] /lessons/:lessonId/reorder - Success', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch(`/lessons/${lesson.id.toString()}/reorder`)
  ...
  test('[PATCH] /lessons/:lessonId/reorder - Lesson not found', async () => {
    const user = await accountFactory.makePrismaAccount();
    const accessToken = jwt.sign({ sub: user.id.toString() });

    const response = await request(app.getHttpServer())
      .patch('/lessons/non-existent-lesson/reorder')
      .set('Authorization', `Bearer ${accessToken}`)
      .send({
        position: 5,
      });
  ...
  test('[PATCH] /lessons/:lessonId/reorder - Not the course creator', async () => {
  ...
  test('[PATCH] /lessons/:lessonId/reorder - Invalid position', async () => {
    const user = await accountFactory.makePrismaAccount();
    const accessToken = jwt.sign({ sub: user.id.toString() });
