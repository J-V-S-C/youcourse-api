...
describe('Delete Unit (E2E)', () => {
  ...
  test('[DELETE] /units/:unitId - Success', async () => {
  ...
  test('[DELETE] /units/:unitId - Unit not found', async () => {
    const user = await accountFactory.makePrismaAccount();
    const accessToken = jwt.sign({ sub: user.id.toString() });

    const response = await request(app.getHttpServer())
      .delete('/units/non-existent-unit')
      .set('Authorization', `Bearer ${accessToken}`);

    expect(response.statusCode).toBe(400);
  });

  test('[DELETE] /units/:unitId - Not the course creator', async () => {
  ...
  test('[DELETE] /units/:unitId - Without auth', async () => {
    const user = await accountFactory.makePrismaAccount();
